...
- **Balance tool (M12):** `npx tsx scripts/balance.ts [years] [careful,greedy,neglectful] [--difficulty easy|normal|hard] [--scale tax=1.5,upkeep=0.8] [--csv dir] [--verbose N]` (~1 min per 20-year careful run). Env `MOOD=1` (mood factors per zone at the end), `ADVICE=1`, `DEMAND=1` (vacancy and demand factors), `GARB=1` (garbage facilities).
