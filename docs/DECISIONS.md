...
- The flat formation reaches 8 m beyond the shoulder (a bench). The terrain is 8 m height samples and the road is draped on them, so a narrower formation left the road tilted across wherever side slopes started inside the cells under it. The bench also gives the first row of lots a level site at road level.
...
- Earthworks cost $0.40 per cubic metre moved. On the sample, that adds a median 16 % to a street on gentle ground (cross-slopes are benched) and 85 % where the ground is 15 % or steeper: a trade-off, not a wall. Bulldozing refunds a quarter of the road but not its earthworks; undo refunds everything.
...
- Terrain edits are saved as a full 257² Float32 delta rather than a sparse list: simplest to keep exact, gzip shrinks the zeros, and M24's terraforming reuses it. Heights are always computed as seed + delta, live and on load, so save/load and undo reproduce the ground bit for bit (an early version stored the target heights and drifted by a float ulp). SAVE_VERSION 12; older saves migrate to an all-zero delta and keep their ground.
